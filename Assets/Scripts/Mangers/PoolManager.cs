...
public class PoolManager
{
    ...
    public GameObject GetObject(Transform postion)
    {
        GameObject obj = _objects.FirstOrDefault(x => !x.activeSelf);
        if (obj == null) obj = Create();
        obj.transform.position = postion.position;
        obj.SetActive(true);
        return obj;
    }
    ...
    {
        GameObject obj = MonoBehaviour.Instantiate(_prefab, _startPosition.position,Quaternion.identity);
        obj.SetActive(false);
        _objects.Add(obj);
        return obj;
    }
